package com.example.mdp_android_grp4;
...
import com.example.mdp_android_grp4.ui.main.BluetoothConnectionService;
import com.example.mdp_android_grp4.ui.main.BluetoothPopUp;
import com.example.mdp_android_grp4.ui.main.CommsFragment;
import com.example.mdp_android_grp4.ui.main.GridMap;
import com.example.mdp_android_grp4.ui.main.MapInformation;
import com.example.mdp_android_grp4.ui.main.MapTabFragment;
import com.example.mdp_android_grp4.ui.main.ReconfigureFragment;
import com.example.mdp_android_grp4.ui.main.SectionsPagerAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        printMDFStringButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                message = "Obstacle : " + GridMap.getPublicMDFObstacle();
                editor.putString("message", CommsFragment.getMessageReceivedTextView().getText() + "\n" + message);
                editor.commit();
                refreshMessageReceived();
            }
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            try {
                if (message.length() > 8 && message.substring(2,7).equals("image") && (Integer.parseInt(message.substring(14,15))!=0 && Integer.parseInt(message.substring(15,16))>=0 && Integer.parseInt(message.substring(15,16))<=5)) {
                    //&& (message.indexOf(15)>=1 && message.indexOf(15)<=15)) {
                    //showLog("lol" + message.indexOf(15));
                    //MainActivity.printMessage("here"+ message.substring(14,15));
                    JSONObject jsonObject = new JSONObject(message);
                    JSONArray jsonArray = jsonObject.getJSONArray("image");
                    gridMap.drawImageNumberCell(jsonArray.getInt(0),jsonArray.getInt(1),jsonArray.getInt(2));
                    GridMap.setObstacleCoord(jsonArray.getInt(0)+1,jsonArray.getInt(1)+1);
            ...
            if (gridMap.getAutoUpdate() || MapTabFragment.manualUpdateRequest) {
                try {
                    ...
                    try {
                        JSONObject msgJson = gridMap.processAlgoMsg(message);
                        gridMap.setReceivedJsonObject(msgJson);
                        gridMap.updateMapInformation();
                        MapTabFragment.manualUpdateRequest = false;
                        showLog("messageReceiver: try decode successful");
